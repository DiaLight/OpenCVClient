...
namespace cli {
    ...
    public:
        ...
        void set_default(bool is_required, const std::string& description = "") {
            CmdBase *command = new CmdArgument<T> { "", "", description, is_required };
            _commands.push_back(command);
        }

        template<typename T>
        void set_required(const std::string& name, const std::string& alternative, const std::string& description = "") {
            CmdBase *command = new CmdArgument<T> { name, alternative, description, true };
            _commands.push_back(command);
        }
