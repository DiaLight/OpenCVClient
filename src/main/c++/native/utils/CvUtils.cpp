...
bool CvUtils::linesCompare(Line4i const &l1, Line4i const &l2, int threshold) {
...
bool CvUtils::pointsCompare(Point2i const &p1, Point2i const &p2, int threshold) {
    return abs(p1.x - p2.x) <= threshold &&
           abs(p1.y - p2.y) <= threshold;
}

void CvUtils::filterPoints(const vector<Point2i> &src, vector<Point2i> &dst, int threshold) {
...
void CvUtils::filterLines(const vector<Line4i> &src, vector<Line4i> &dst, int threshold) {
    // отсеиваем "похожие" отрезки
    for(auto const& l1 : src) {
        bool found = false;
        for(auto const& l2 : dst) {
...
Line4i CvUtils::testTriangle(const Line4i &l1, const Line4i &l2, int threshold) {
...
void ::CvUtils::filterContoursBySize(vector<vector<Point>> const &in, vector<vector<Point>> &out, int def_min_size, int def_max_size) {
...
void ::CvUtils::contoursToLines(vector<vector<Point>> const &in, vector<vector<Line4i>> &out) {
    for(auto const& contour : in) {
